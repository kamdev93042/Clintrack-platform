...
export default function ClinicOwnerVerifyEmailScreen() {
  const params = useLocalSearchParams<{ email: string; registrationToken: string }>();
  const email = Array.isArray(params.email) ? params.email[0] : params.email;
  const registrationToken = Array.isArray(params.registrationToken) ? params.registrationToken[0] : params.registrationToken;
  ...
  useEffect(() => {
    if (!email || !registrationToken) {
      Alert.alert('Error', 'Missing registration information. Please start registration again.');
      router.replace('/clinic-owner-signup');
    }
  ...
  const handleVerifyEmail = async () => {
    ...
    setLoading(true);
    try {
      const result = await ClinicOwnerAuthService.completeRegistration(registrationToken, email, otpString);
      ...
      setLoading(false);
      setErrors({ otp: error.message || 'Failed to verify email. Please try again.' });
    }
  };
  ...
  const handleResendOTP = async () => {
    ...
    // Note: Resending OTP would require re-initiating registration
    // For simplicity, we'll show a message to restart registration
    Alert.alert(
      'Resend OTP',
      'To resend OTP, please go back and restart the registration process.',
      [
        {
          text: 'Go Back',
          onPress: () => router.replace('/clinic-owner-signup')
        },
        {
          text: 'Cancel',
          style: 'cancel'
      ...
      {loading && (
        <View style={styles.loaderOverlay}>
          <View style={styles.loaderContainer}>
            ...
            <Text style={styles.loaderText}>Verifying email...</Text>
          </View>
        </View>
      )}
