...
class ClinicOwnerSessionService {
  ...
  async createSession(patientId, sessionData, files = { photos: [], videos: [] }) {
    try {
      ...
      const response = await ApiService.postFormData(`/clinic-owner/sessions/${patientId}`, formData);
      return {
        success: true,
...
export default new ClinicOwnerSessionService();
